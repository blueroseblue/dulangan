...
if (isset($_GET['id'])) {
    ...
    $sql = "UPDATE accounts SET is_accepted = 1 WHERE account_id = ?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                ...
                if (mysqli_stmt_execute($phoneStmt)) {
                    ...
                    header('location: accounts.php?status=accepted');
                    exit;
                ...
                echo "No rows were updated. Please check if the account ID exists and if `is_accepted` is already set to 1.";
            }
        } else {
